...
st.markdown("Herramienta de análisis para el monitoreo de precisión desarrollado por CIATEC.")
st.markdown("---")

# 2. Datos simulados (En caso de no subir archivo)
@st.cache_data
...
# 3. PANEL LATERAL: CONFIGURACIÓN Y PARÁMETROS
...
textura = st.sidebar.selectbox(
    ...
    index=1,
    help="Define el punto de partida del umbral crítico basado en la retención hídrica del suelo."
)

# Inicialización segura del estado (Crucial para que no se borre nada)
if 'umbral_dinamico' not in st.session_state:
    st.session_state.umbral_dinamico = pmp_teoricos[textura]

# Botón para resetear al valor teórico de la textura seleccionada
if st.sidebar.button("Aplicar valor de textura"):
    st.session_state.umbral_dinamico = pmp_teoricos[textura]

# AJUSTE FINO DEL UMBRAL (Restaurado)
umbral_final = st.sidebar.slider(
    "Ajuste Fino de Umbral (%)", 
    5.0, 40.0, 
    value=float(st.session_state.umbral_dinamico),
    key="slider_umbral",
    help="Modifica manualmente el Punto de Marchitez Permanente (PMP) para los cálculos predictivos."
)
...
# 6. ANÁLISIS TEMPORAL (Visualización Dual)
st.markdown("### Análisis Temporal")
if tiene_temp:
    col_graf1, col_graf2 = st.columns(2)
    with col_graf1:
        st.plotly_chart(px.line(datos, x='Fecha_Hora', y=['Control_Humedad(%)', 'Estres_Humedad(%)'], 
                                title='Evolución de la Humedad del Suelo'), use_container_width=True)
    with col_graf2:
        st.plotly_chart(px.line(datos, x='Fecha_Hora', y='Temperatura(°C)', 
                                title='Variación de Temperatura Ambiental', color_discrete_sequence=['red']), use_container_width=True)
else:
    st.plotly_chart(px.line(datos, x='Fecha_Hora', y=['Control_Humedad(%)', 'Estres_Humedad(%)'], 
                            title='Evolución de la Humedad del Suelo'), use_container_width=True)

# 7. INTELIGENCIA AGRONÓMICA (Secciones restauradas)
st.markdown("### 📊 Inteligencia Agronómica")
col_stat1, col_stat2, col_stat3 = st.columns(3)

with col_stat1:
    # DIFERENCIA MEDIA (Restaurado)
...
with col_stat2:
    # CORRELACIÓN (Si existe temperatura)
    if tiene_temp:
...
with col_stat3:
    ...
    st.caption("Regla del trapecio. Cuantifica la severidad acumulada del déficit hídrico en el tiempo.")
...
if pendiente < 0:
    ...
    if horas_restantes > 0:
        ...
        st.error(f"🚨 El cultivo ya superó el umbral crítico detectado de **{st.session_state.umbral_dinamico:.1f}%**.")
else:
...
with col_txt:
    st.info(f"**Umbral calibrado actual:** {st.session_state.umbral_dinamico:.1f}%")
    st.caption("Presiona el botón si observas marchitez foliar. El sistema ajustará todas las predicciones automáticamente.")
